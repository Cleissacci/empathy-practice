...
(() => {
  ...
  function load() {
    ...
    // Baked-in seed (written into the page at build/bundle time) so downloaded
    // / shared copies show their images even though the fetch below can't reach
    // the sidecar from a file:// origin. Lowest priority: the live sidecar and
    // any in-memory change both override it.
    const inline = (typeof window !== 'undefined' && window.__IMAGE_SLOTS_INLINE
      && typeof window.__IMAGE_SLOTS_INLINE === 'object') ? window.__IMAGE_SLOTS_INLINE : null;
    loadP = fetch(STATE_FILE)
      .then((r) => (r.ok ? r.json() : null))
      .then((j) => {
        // Merge priority: in-memory change (slots) > live sidecar (j) > seed.
        const disk = Object.assign({}, inline, j);
        if (disk && typeof disk === 'object') {
          const merged = Object.assign({}, disk, slots);
          // A framing-only write that raced ahead of hydration must not
          // drop a user image that's only on disk — inherit u from disk
  ...
  async function toDataUrl(file, targetW) {
    ...
    try { return encodeBitmap(bitmap, targetW); }
  ...
  async function bitmapFromUrl(url) {
    ...
    try { return await createImageBitmap(img); } catch (e) { return null; }
  }
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '    <div class="cap"></div>' +
        '    <div class="sub">drop, <u>browse</u>, or paste</div></div>' +
    ...
    connectedCallback() {
      ...
      this._onEnter = () => { ImageSlot._hover = this; };
      this._onLeave = () => { if (ImageSlot._hover === this) ImageSlot._hover = null; };
      ...
      if (!ImageSlot._pasteBound) {
        ...
        document.addEventListener('paste', (e) => {
          ...
          if (imgItem) {
            ...
            if (f) { e.preventDefault(); slot._ingest(f); return; }
          }
          const txt = (cd.getData && cd.getData('text/plain') || '').trim();
          if (/^https?:\/\/\S+/i.test(txt)) { e.preventDefault(); slot._ingestUrl(txt); }
        });
      }
      ...
      if (e.type === 'dragenter' || e.type === 'dragover') {
        ...
        if (url) {
          this._ingestUrl(url).then((ok) => {
            if (ok) return;
            if (f) this._ingest(f);
            else this._setError('Couldn’t load that image — save it and drop the file instead.');
          });
        } else if (f) {
          this._ingest(f);
        }
    ...
    async _ingestUrl(url) {
      ...
      try {
        if (gen !== this._gen) { bitmap.close && bitmap.close(); return true; }
